<template>
      ...
      <div class="pt-4">
        ...
        <v-data-table :items="student_list" :headers="student_header">
          <template #[`item.action`]="{ item }">
            <v-btn small color="primary" @click="onNewTap(item?.registrationId)">Print</v-btn>
          </template>
        </v-data-table>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      async onGetStudent() {
        try {
          await this.$axios.$get(`/report/registered`, { params: { schoolYear: this.form.schoolYear, levelId: this.form.levelId, classId: this.form.classId } }).then((data) => {
