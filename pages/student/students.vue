<template>
    <div>
        <h3>ຂໍ້ມູນນັກຮຽນ</h3><br />
        <v-row dense>
            <v-col cols="2">
                <v-text-field v-model="form.name" dense outlined label="ເລືອກສົກຮຽນ" hide-details="auto"></v-text-field>
            </v-col>
            <v-col cols="2">
...
export default {
    ...
    methods: {
        async onGetStudent() {
            try {
                await this.$axios.$get(`/report/students`, { params: this.form }).then((data) => {
