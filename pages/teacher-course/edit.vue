        ...
        <v-card>
            ...
            <v-card-text>
                <v-row>
                    ...
                    <v-col cols="3">
                        <v-select v-model="form.subjectId" :items="subjectList" item-text="name" item-value="id" dense
                            outlined label="ເລືອກວິຊາຮຽນ"></v-select>
                    </v-col>
                </v-row>
...
export default {
    ...
    async mounted() {
        try {
            const { levelId, teacherId, schoolYear, subjectId } = this.$route.query;
            this.$axios.$get(`/manage/level-class-teacher-id`, {
                params: {
                    levelId, teacherId, schoolYear, subjectId
                }
            }).then((data) => {
                this.form = data
            })
        ...
        await this._onGetYear()
        await this.onGetTeacher()
    ...
    methods: {
        async _onGetLevel(id) {
            try {
                await this.$axios.$get(`/registration/level-by-school-year/${id}`).then((data) => {
                    this.levelList = data
                    this._onGetClass(this.form.levelId)
                    console.log(data)
                })
            } catch (error) {
                swal.fire({
        ...
        async onGetTeacher() {
            try {
                await this.$axios.$get('/teacher', { params: { type: 'teacher' } }).then((data) => {
                    ...
                    this._onGetLevel(this.form.schoolYear)
                })
            } catch (error) {
                swal.fire({
                    icon: 'error',
        ...
        async onInser() {
            try {
                await this.$axios.$put(`/manage/level-class-teacher`, this.form).then((data) => {
                    swal.fire({
                        icon: 'success',
                        text: 'ສຳເລັດ'
                    })
