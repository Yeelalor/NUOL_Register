        ...
        <v-card width="800">
            <v-card-text>
                <div style="display:flex;">
                    ...
                    <v-btn color="primary" to="/class/create">ເພີ່ມລາຍການ</v-btn>
                </div>
                <v-data-table :items="list" :headers="headers" dense>
                    <template #[`item.actions`]="{ item }">
                        <v-btn color="primary" small dark v-bind="attrs" v-on="on"
                            @click="_onGetForUp(item.id, item.name, item?.schoolYear, item?.level)">
                            ແກ້ໄຂ
                        </v-btn>
            ...
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-select v-model="form.schoolYear" :items="yearList" item-text="name" item-value="id" dense outlined
                        label="ເລືອກສົກຮຽນ"></v-select>
                    <v-select v-model="form.level" :items="levelList" dense outlined item-text="name" item-value="id"
                        label="ຊື່ຊັ້ນຮຽນ"></v-select>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
        ...
        async _onUpdate() {
            try {
                await this.$axios.$put(`/manage/class/${this.key}`, this.form).then(() => {
                    swal.fire({
                        icon: 'success',
                        text: 'ສຳເລັດ'
                    })
        ...
        async onDelete(item) {
            this.key = item;
            try {
                await this.$axios.$delete(`/manage/class/${this.key}`, this.form).then(() => {
                    swal.fire({
                        icon: 'success',
                        text: 'ສຳເລັດ'
                    })
