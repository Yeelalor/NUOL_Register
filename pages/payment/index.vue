    ...
    <div style="max-width: 800px" v-if="registration.invoice.length != 0">
      ...
      <v-simple-table>
        ...
        <tbody>
          <tr>
            <td>
              {{ registration.detail.name }} {{ registration.detail.surname }}
            </td>

            <td>{{ registration.detail.gender }}</td>
            <td>
              {{
                registration.detail.birthday
                  ? $moment(registration.detail.birthday).format('DD-MM-YYYY')
                  : '-'
              }}
            </td>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="font-weight-bold">
              {{ sumTotal(registration?.invoice) }} ກີບ
            </td>
          </tr>
        </tbody>
      </v-simple-table>
      ...
      <div class="pt-10">
        <v-btn
          ...
          rounded
          @click="onConfirm"
          v-if="registration?.detail?.paymentStatus != 'paid'"
          >ຢືນຢັນການຈ່າຍເງິນ</v-btn
        >
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    onConfirm() {
      try {
        this.$axios.put('/registration/complete/' + this.key).then((data) => {
          swal.fire({
            icon: 'success',
            text: 'ສຳເລັດ',
          })
